...
func TestDirectTransferSettlement(t *testing.T) {
	waitForHealthy(t)
	taro, taroID := login(t, "taro", "taro-password")
	hanako, hanakoID := login(t, "hanako", "hanako-password")
	...
	// 収入同額・支出なし → 精算分0。立替精算純額 太郎→花子 (5000-2000)=3000。
	if status, _ = doJSON(t, http.MethodPut, "/months/"+month+"/incomes/"+taroID, taro, map[string]any{"amountYen": 100000}); status != http.StatusOK {
		t.Fatalf("income(taro) status = %d", status)
	}
	if status, _ = doJSON(t, http.MethodPut, "/months/"+month+"/incomes/"+hanakoID, hanako, map[string]any{"amountYen": 100000}); status != http.StatusOK {
		t.Fatalf("income(hanako) status = %d", status)
	}
