...
export default function SettlementScreen({ month, members, notify, onError, onNavigate }) {
  ...
  const { loading, data, error, reload } = useAsync(async () => {
    ...
    try {
      settlement = await api("GET", `/months/${month}/settlement`);
    } catch (e) {
      settlementError = e;
    }
    return {
  ...
  const setSettled = async (value, celebrate) => {
    setBusy(true);
    try {
  ...
  const itemsFor = (memberId) => {
    const oneOff = data.expenses
