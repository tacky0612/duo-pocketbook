...
// 変動費・固定費のサブ画面で共通して使う props（画面遷移系は使わないので絞る）。
...
// 現在の日付（ローカル）を YYYY-MM-DD で返す。支出日入力の初期値に使う。
...
// --- 変動費（対象月の単発支出） ---
function VariableExpenses({ month, members, me, notify, onError }: SectionProps & { month: YearMonth }) {
    ...
    <div className="grid gap-4 lg:grid-cols-5 lg:items-start">
      <Card className="lg:col-span-2 lg:sticky lg:top-20">
        ...
        <p className="mb-4 text-sm text-slate-500 dark:text-slate-400">
          食費・日用品など、その月だけの共有支出です。指定した日付が属する月の精算に含まれます。
        </p>
...
// --- 固定費（毎月発生する共有支出。すべての月の精算に自動で含まれる） ---
function FixedExpenses({ members, me, notify, onError }: SectionProps) {
