...
`api/openapi.yaml` がこの層の契約であり、エンドポイント変更時は必ず同期する。
...
React + Vite + Tailwind CSS。UIはコンポーネント（`src/components/`）で組み立てており、API呼び出しとセッション管理は `src/api.js` に集約している。`App.jsx` がデータ取得と状態を保持し、各コンポーネントへ渡す。
