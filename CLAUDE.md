...
- **API 仕様**: `api/openapi.yaml`（OpenAPI 3.1）が外部ツール連携の契約。**Go コード（`internal/web` の swag 注釈・DTO 型）が正**で、`make openapi`（swag）で自動生成する。手で編集しない
...
- **web 層** (`internal/web/`): API インターフェイス（ハンドラ、ルーティング、JWT認証、CORS）。ハンドラの swag 注釈と DTO 型が OpenAPI 定義の生成元（source of truth）
...
- API変更時は `internal/web` のハンドラ注釈・DTO を更新し、`make openapi` で `api/openapi.yaml` を再生成する（手で編集しない）。`docs/api.md` も更新する。CI の `openapi-check` ジョブが未再生成を検出して失敗させる
...
make openapi-check                      # openapi.yaml がコードと同期しているか検証（CI）
