...
duo-pocketbook は夫婦2人で使う家計簿 Web アプリケーション。2アカウントのみからのアクセスを想定し、共有支出を登録し、月次で双方の収入を入力して精算額を算出する。
...
- **言語**: Go（バックエンド）、React + Vite + Tailwind CSS（`frontend/`。UIはコンポーネントで組み立てる。CSSは書かずTailwindユーティリティを使う）
